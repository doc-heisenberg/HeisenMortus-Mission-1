//This script is a script written by Tophe to allow dynamic respawning of vehicles. The following is a direct copy and paste with instuctions on how to implement this script. It is used to allow for vehicles to respawn.
				...
				*/
				...
				{
				sleep (2 + random 10);
				if ((getDammage _unit > 0.:sick: and ({alive _x} count crew _unit == 0)) then {_dead = true};

				// Check if the vehicle is deserted.
				if (_deserted > 0) then
				{
				if ((getPosASL _unit distance _position > 10) and ({alive _x} count crew _unit == 0) and (getDammage _unit < 0.:sick:) then
				{
